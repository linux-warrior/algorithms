# https://contest.yandex.ru/contest/24414/run-report/141568927/
#
# -- Принцип работы --
#
...
#
# Во время поиска переданный запрос разбивается на уникальные слова, после чего в цикле формируется
# словарь результатов, сопоставляющий порядковые номера документов и их релевантность. Релевантность
# документа определяется как сумма значений релевантности всех слов из поискового запроса, которые в
# нем содержатся. Далее список элементов словаря результатов поиска сортируются по убыванию релевантности
# и возрастанию порядкового номера документа, после чего функция возвращает заданное количество элементов
# из списка (по умолчанию 5).
#
...
# документе. Поскольку при поиске выполняется сортировка результатов по релевантности, то временная
# сложность обработки поискового запроса составляет `O(query_words_count · documents_count +
# documents_count · log(documents_count))`, где `query_words_count` — количество уникальных слов в запросе.
#
...
# формируется сначала словарь, а затем и список, сопоставляющий документы и их релевантность, то
# пространственная сложность поиска составляет `O(documents_count)`.
