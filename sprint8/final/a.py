...
# от исходных данных, но в среднем его можно оценить примерно как `O(log |sᵢ|)`, поскольку чем длиннее
# строка, тем больше вложенных циклов повторений символов она теоретически может включать. Также в
# методе `CommonPrefixTool.get_longest_common_prefix()` из символов создается строка, содержащая
# наибольший общий префикс длиной порядка `O(min(|sᵢ|))`. Таким образом, пространственная сложность
# алгоритма составляет `O(min(|sᵢ|) + Σ log |sᵢ|)`.
