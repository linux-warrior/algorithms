...
class Graph:
    ...
    def get_vertices_times(self, start_vertex: int) -> Sequence[VertexTime]:
        dfs = DFS(self)
...
class GetVerticesTimesVisitor(DFSVisitor):
    vertices_times: list[VertexTime]

    def __init__(self, *, vertices_count: int) -> None:
        self.vertices_times = [VertexTime() for _vertex in range(vertices_count)]
        self.current_time = 0

    def get_vertices_times(self) -> Sequence[VertexTime]:
        return self.vertices_times

    def start_handle_vertex(self, vertex: int) -> None:
        self.vertices_times[vertex].entry = self.current_time
        self.current_time += 1

    def end_handle_vertex(self, vertex: int) -> None:
        self.vertices_times[vertex].exit = self.current_time
        self.current_time += 1
...
def main() -> None:
    ...
    for vertex_time in vertices_times:
        print(vertex_time.entry, vertex_time.exit)
